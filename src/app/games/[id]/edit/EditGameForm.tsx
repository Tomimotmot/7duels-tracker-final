...
import { Player, WinCondition, GameWithPlayers } from '@/types/database.types'
...
export default function EditGameForm({ game, players, winConditions }: Props) {
  const router = useRouter()
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [p1, setP1] = useState(game.player_1_id)
  const [p2, setP2] = useState(game.player_2_id)
  const [winner, setWinner] = useState(game.winner_id || '')
  const [winCond, setWinCond] = useState<number | null>(game.win_condition_id)
  ...
  const [p1Scores, setP1Scores] = useState<Record<ScoreKey, number>>({
    blue_cards: game.p1_score_blue_cards,
    green_cards: game.p1_score_green_cards,
    yellow_cards: game.p1_score_yellow_cards,
    purple_cards: game.p1_score_purple_cards,
    wonders_points: game.p1_score_wonders_points,
    progress_points: game.p1_score_progress_points,
    military_points: game.p1_score_military_points,
    coin_points: game.p1_score_coin_points,
    gods_cards: game.p1_score_gods_cards,
    senator_points: game.p1_score_senator_points,
    great_temple: game.p1_score_great_temple,
    thalassa_points: game.p1_score_thalassa_points || 0,
  })
  const [p2Scores, setP2Scores] = useState<Record<ScoreKey, number>>({
    blue_cards: game.p2_score_blue_cards,
    green_cards: game.p2_score_green_cards,
    yellow_cards: game.p2_score_yellow_cards,
    purple_cards: game.p2_score_purple_cards,
    wonders_points: game.p2_score_wonders_points,
    progress_points: game.p2_score_progress_points,
    military_points: game.p2_score_military_points,
    coin_points: game.p2_score_coin_points,
    gods_cards: game.p2_score_gods_cards,
    senator_points: game.p2_score_senator_points,
    great_temple: game.p2_score_great_temple,
    thalassa_points: game.p2_score_thalassa_points || 0,
  })
  ...
  return (
    <form onSubmit={submit} className="space-y-6">
      ...
      {/* Player Selection */}
      <div>
        <label className="block text-sm font-medium text-[var(--gold)] mb-3">Spieler</label>
